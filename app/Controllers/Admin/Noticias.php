...
class Noticias extends BaseController
{
  ...
  public $imagebase = 'static/images/noticias/';
  public $imagebaseImagenes = 'static/images/imagenes/';

  public function __construct()
  {
    $this->model = new GeneralModel('noticias');
  ...
  public function guardar($id = '')
  {
    ...
    $this->dieMsg(true, '', base_url('admin/noticias'));
  }
  ...
  public function crear()
  {
    ...
    $this->addJs(array(
      ...
      'js/admin/noticias/form.js',
    ));
    $datos['id'] = '0';
    $datos['titulo'] = 'Nueva noticia';
  ...
  public function accion($id, $tipo, $no = '')
  {
    ...
    if ($tipo == 'up') {
      ...
      $this->db->query("SELECT orden INTO @current_order FROM noticias WHERE id = {$id};");
      $row = $this->db->query("SELECT id, orden FROM noticias WHERE orden < @current_order ORDER BY orden DESC LIMIT 1;")->getRow();
      if ($row) {
        $this->db->query("SELECT id, orden INTO @swap_id, @swap_order FROM noticias WHERE orden < @current_order ORDER BY orden DESC LIMIT 1;");
        $this->db->query("UPDATE noticias SET orden = @swap_order WHERE id = {$id};");
        $this->db->query("UPDATE noticias SET orden = @current_order WHERE id = @swap_id;");
      }
    ...
    if ($tipo == 'down') {
      $this->db->query("START TRANSACTION;");
      $this->db->query("SELECT orden INTO @current_order FROM noticias WHERE id = {$id};");
      $row = $this->db->query("SELECT id, orden FROM noticias WHERE orden > @current_order ORDER BY orden ASC LIMIT 1;")->getRow();
      if ($row) {
        $this->db->query("SELECT id, orden INTO @swap_id, @swap_order FROM noticias WHERE orden > @current_order ORDER BY orden ASC LIMIT 1;");
        $this->db->query("UPDATE noticias SET orden = @swap_order WHERE id = {$id};");
        $this->db->query("UPDATE noticias SET orden = @current_order WHERE id = @swap_id;");
      }
      $this->db->query("COMMIT;");
    ...
    if ($tipo == 'upup') {
      $this->db->query("START TRANSACTION;");
      $this->db->query("SELECT orden INTO @current_order FROM noticias WHERE id = {$id};");
      $this->db->query("UPDATE noticias SET orden = orden + 1 WHERE orden < @current_order;");
      $this->db->query("UPDATE noticias SET orden = 1 WHERE id = {$id};");
      $this->db->query("COMMIT;");
    }
