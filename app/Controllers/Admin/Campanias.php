...
class Campanias extends BaseController
{
  ...
  public $imagebase = 'static/images/campanias/';
  public $imagebaseImagenes = 'static/images/imagenes/';

  public function __construct()
  {
    $this->model = new GeneralModel('campanias');
  ...
  public function index()
  {
    ...
    $funcOrden = function ($d, $row) {
      $url = base_url("admin/{$this->controller}/accion/" . $row['id']);
      return '<div class="btn-group" role="group" aria-label="Default button group">
        <a href="' . $url . '/up" class="btn btn-outline-secondary bup"><i class="fa-solid fa-chevron-up"></i></button>
        <a href="' . $url . '/down" class="btn btn-outline-secondary bdown"><i class="fa-solid fa-chevron-down"></i></button>
        <a href="' . $url . '/upup" class="btn btn-outline-secondary bupup"><i class="fa-solid fa-arrows-up-to-line"></i></button>
      </div>';
    };
    helper('formulario');

    $columns = array(
      array('db' => 'orden', 'dt' => 'Ord', "field" => "orden"),
      array('db' => 'orden', 'dt' => 'Orden', "field" => "orden", "formatter" => $funcOrden),
      array('db' => 'titulo', 'dt' => 'Titulo', "field" => "titulo"),
      array('db' => 'fecha', 'dt' => 'Fecha', "field" => "fecha", "formatter" => fn($d) => dateToUser($d)),
      array('db' => 'id',  'dt' => 'Galeria', "field" => "id", "formatter" => $funcGaleria),
      array('db' => 'imagen', 'dt' => 'Imagen', "field" => "imagen", "formatter" => $funcImagen),
      array('db' => 'estado', 'dt' => 'Activo', "field" => "estado", "formatter" => $botonActivo),
  ...
  public function guardar($id = '')
  {
    ...
    if (empty($id)) {
      ...
      $this->model->update(['id' => $id], ['orden' => $id]);
      $this->accion($id, 'upup', '-');
    } else {
    ...
    $this->dieMsg(true, '', base_url('admin/campanias'));
  }
  ...
  public function crear()
  {
    ...
    $this->addJs(array(
      ...
      'js/admin/campanias/form.js',
    ));
    $datos['id'] = '0';
    $datos['titulo'] = 'Nueva campaña';
  ...
  public function accion($id, $tipo, $no = '')
  {
    $this->dieAjax();

    if ($tipo == 'up') {
      $this->db->query("START TRANSACTION;");
      $this->db->query("SELECT orden INTO @current_order FROM campanias WHERE id = {$id};");
      $row = $this->db->query("SELECT id, orden FROM campanias WHERE orden < @current_order ORDER BY orden DESC LIMIT 1;")->getRow();
      if ($row) {
        $this->db->query("SELECT id, orden INTO @swap_id, @swap_order FROM campanias WHERE orden < @current_order ORDER BY orden DESC LIMIT 1;");
        $this->db->query("UPDATE campanias SET orden = @swap_order WHERE id = {$id};");
        $this->db->query("UPDATE campanias SET orden = @current_order WHERE id = @swap_id;");
      }
    ...
    if ($tipo == 'down') {
      $this->db->query("START TRANSACTION;");
      $this->db->query("SELECT orden INTO @current_order FROM campanias WHERE id = {$id};");
      $row = $this->db->query("SELECT id, orden FROM campanias WHERE orden > @current_order ORDER BY orden ASC LIMIT 1;")->getRow();
      if ($row) {
        $this->db->query("SELECT id, orden INTO @swap_id, @swap_order FROM campanias WHERE orden > @current_order ORDER BY orden ASC LIMIT 1;");
        $this->db->query("UPDATE campanias SET orden = @swap_order WHERE id = {$id};");
        $this->db->query("UPDATE campanias SET orden = @current_order WHERE id = @swap_id;");
      }
      $this->db->query("COMMIT;");
    ...
    if ($tipo == 'upup') {
      $this->db->query("START TRANSACTION;");
      $this->db->query("SELECT orden INTO @current_order FROM campanias WHERE id = {$id};");
      $this->db->query("UPDATE campanias SET orden = orden + 1 WHERE orden < @current_order;");
      $this->db->query("UPDATE campanias SET orden = 1 WHERE id = {$id};");
      $this->db->query("COMMIT;");
    }
    if (empty($no)) $this->dieMsg();
  }
}
