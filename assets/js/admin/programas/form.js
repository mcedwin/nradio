$("#contenido").val(wpautop($("#contenido").val()))
tinymce.init({
  ...
  toolbar: [
    // Formato de texto
    'undo redo | ' +
    'styles | fontfamily fontsize | ' + // Tipos de encabezados
    
    // Estilos de texto
    'bold italic underline | ' + // Negrita, cursiva, subrayado
    'backcolor | forecolor | ' + // Color de texto y fondo
...
$("#formContent").load_img()

//tinymce.activeEditor.execCommand('mceInsertContent', false, wpautop($("#contenido").val()));
$("#formOptions").on("submit", function () {
  ...
  $(this).mysave((data) => {
    $("#result").val(data);
    tinymce.activeEditor.execCommand("mceInsertContent", false, wpautop(data.data));
  });
  return false;

  /*
  $.post("lib/webgpt.php", $("#formOptions").serialize())
    .done(function(data) {
      $('#result').val(data)
      tinymce.activeEditor.execCommand('mceInsertContent', false, wpautop(data));
    });
  return false;*/
});
